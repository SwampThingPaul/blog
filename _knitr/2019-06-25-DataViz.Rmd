...
green.melt.POR.stats=ddply(subset(green.melt,CY%in%seq(1979,2018,1)),"DOY",summarise,
...
```{r ,echo=T,message=F,warning=F}
alpha=0.95;
greenland.melt.mean=ddply(subset(green.melt,CY%in%seq(1979,2018,1)),"CY",summarise,
                          mean.val=mean(MeltArea.sqkm,na.rm=T),
                          sd.val=sd(MeltArea.sqkm,na.rm=T),
                          N.val=N(MeltArea.sqkm))

greenland.melt.mean$Df=greenland.melt.mean$N.val-1; #Degree of freedom
greenland.melt.mean$Tp=with(greenland.melt.mean,abs(qt(1-alpha,Df))); # test statistic
greenland.melt.mean$LCI=with(greenland.melt.mean,mean.val-sd.val*(Tp/sqrt(N.val))); #Lower CI
greenland.melt.mean$UCI=with(greenland.melt.mean,mean.val+sd.val*(Tp/sqrt(N.val))); #Upper CI
