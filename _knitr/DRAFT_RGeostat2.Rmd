...
...and for my final trick we will use some data from the United States Environmental Protection Agency from their [Regional Environmental Monitoring and Assessment Program](https://www.epa.gov/everglades/environmental-monitoring-everglades){target="blank"}.  
...
```{r f11-diff-patterns, fig.cap="Three different point patterns: a single cluster, a dual cluster and a randomly scattered pattern.", fig.height=2.25, fig.width=4.5, echo=FALSE,fig.align="center"}
...
Selection of a K value can also be corrected for various edge corrections, some of you may be heard of *Ripley's K*-function (Ripley 1977). For purposes of th is post we will not delve into this aspect of spatial analysis however if your intrested and eager to learn Gimond (2018) introduces some of these concepts in detail.
...
<center>
![](https://media.giphy.com/media/3E1yVKeqS9JGcHLm0O/200w_d.gif)
</center>

Calculating Moran's I statistic by hand. Code adapted from [rspatial.org](https://rspatial.org/analysis/3-spauto.html){target="_blank"}.
```{r moranI code}
